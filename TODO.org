* CORE
** TODO move tracking logic to core
** TODO implement shallow builds
** TODO implement partial builds
** TODO consider using HashSet for build/nodes and build/links 
** TODO Consider separating link and frontmatter gathering from compilation, to avoid double compilation
for markdown though, where we need a parser, do we parse it twice? hold the ast somewhere?
* CLI
...
** TODO the new command should perhaps also add it's new node to build/nodes.toml 
...
** TODO Add a ~private~ property to nodes
* Tasks
