...
Next: ~@omni.matrix-operations:addition~
0. the link is of form ~name:heading~
1. find the node in db with name ~matrix-operations~
2. the node in question is ~cs/linear-algebra/matrix.typ#operations~
3. now append the ~heading~ /(which keep in mind may or may not exist)/ to the final link
4. final resolved link =cs/linear-algebra/matrix.typ#operations#addition=.
